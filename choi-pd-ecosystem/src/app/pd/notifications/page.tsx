...
export default function PdNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/notifications?userType=pd');
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const response = await fetch('/api/notifications', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ markAllAsRead: true, userType: 'pd' }),
      });
              ...
              <Table>
                ...
                <TableBody>
                  {filteredNotifications.map((notification) => (
                    <TableRow
                      key={notification.id}
                      className={!notification.isRead ? 'bg-purple-50/50' : ''}
                    >
                      <TableCell>
                        {notification.isRead ? (
                          <Badge variant="secondary">읽음</Badge>
                        ) : (
                          <Badge className="bg-purple-100 text-purple-800">안읽음</Badge>
                        )}
                      </TableCell>
                      <TableCell>
