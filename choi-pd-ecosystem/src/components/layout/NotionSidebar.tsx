...
export function NotionSidebar() {
  ...
  if (!isSidebarOpen) {
    return (
      ...
      >
        {/* 접힌 아이콘 바 */}
        <aside className="w-10 h-full border-r border-gray-200 bg-gray-50/80 flex flex-col items-center py-3 gap-2">
          <button
          ...
          {/* 접힌 상태에서 주요 아이콘 표시 */}
          {sections.flatMap(s => s.items).slice(0, 6).map((item, i) => (
            <Link
              key={i}
              href={item.href || '#'}
              className={`p-1.5 rounded-md transition ${pathname === item.href ? 'bg-blue-100 text-blue-600' : 'text-gray-400 hover:bg-gray-200 hover:text-gray-700'}`}
              title={item.label}
            >
              {item.icon}
            </Link>
        ...
        {/* Hover 시 펼쳐지는 오버레이 사이드바 */}
        {isHoverExpanded && (
          <aside
            className="absolute left-0 top-0 z-50 h-full w-56 bg-white border-r border-gray-200 shadow-xl flex flex-col animate-in slide-in-from-left-2 duration-150"
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
          >
            {/* Header */}
    ...
    <aside className="notion-sidebar relative group flex flex-col">
      ...
      <div className="px-2 pb-1">
        ...
        >
          ...
          <span>공개 사이트 보기</span>
          <svg className="w-3 h-3 ml-auto opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
        </a>
      </div>
